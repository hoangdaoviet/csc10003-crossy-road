...
bool simulate_credits(HDC hdc, Input* input, float fTimeSinceStart, float fElapsedTime, Sprite *credits)
{
    clear_screen(0x000000);
    credits->DrawCredits(0, 0, 1200, 700);
...
void RunCreditsLoop(HDC hdc)
{
    Input input = {};
    // Initialize game objects and other necessary variables here
    Sprite credits("CREDITSMENU.txt");
    generateMap();
...
input.buttons[b].changed = true;\
        ...
        fSinceStart += fElapsedTime;
        bool dead = simulate_credits(hdc, &input, fSinceStart, fElapsedTime, &credits);
        if (dead)
            Sleep(1000);
